...
## Penguin sizes {.tabset}

### Bill length ~ body mass of penguins
```{r, warning = FALSE, fig.cap = "**Fig. 1:** Penguin bill length ~ body mass for Adelie, Chinstrap, & Gentoo Penguins", fig.alt = "A scatterplot of body mass in grams by bill length in millimeters for Adelie, Chinstrap, and Gentoo penguins, where we show that penguins with greater body mass have bigger bills."}
...
penguin_summary <- penguins %>% 
  ...
  group_by(species) %>% 
  summarize(
    mean_body_mass = round(mean(body_mass_g, 4)), 
    sd_body_mass = round(sd(body_mass_g, 4)),
    mean_bill_length = round(mean(bill_length_mm, 4)),
    sd_bill_length = round(sd(bill_length_mm, 4))
  )
